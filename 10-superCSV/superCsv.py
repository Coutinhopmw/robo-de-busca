...
def main():
    ...
    arquivo_saida = os.path.join(os.path.dirname(__file__), "superCSV.csv")
    ...
    for caminho_csv in arquivos_csv:
        try:
            # Detecta coluna de referência
            df_temp = pd.read_csv(caminho_csv, nrows=1)
            if 'username_curtiu' in df_temp.columns:
                coluna_ref = 'username_curtiu'
            elif 'username' in df_temp.columns:
                coluna_ref = 'username'
            else:
                print(f"Arquivo ignorado, sem coluna de referência: {caminho_csv}")
                continue

            # Aplica tratamento de duplicatas usando a coluna correta
            df_tratado = remover_duplicatas_curtidas(caminho_csv, coluna_referencia=coluna_ref, manter_primeiro=True)
            if df_tratado is None:
                print(f"Arquivo ignorado por erro: {caminho_csv}")
                continue
            # Renomeia colunas para padronizar
            if 'username_curtiu' in df_tratado.columns:
                df_tratado = df_tratado.rename(columns={'username_curtiu': 'username'})
            ...
            if 'url_foto_perfil' in df_tratado.columns:
                df_tratado['foto_perfil'] = df_tratado['url_foto_perfil'].apply(lambda x: 1 if pd.notnull(x) and str(x).strip() != '' else 0)
                df_tratado = df_tratado.drop(columns=['url_foto_perfil'])

            # Adiciona coluna de origem
            df_tratado['arquivo_origem'] = os.path.basename(caminho_csv)
            # Adiciona data da leitura
            df_tratado['data_leitura'] = data_leitura
